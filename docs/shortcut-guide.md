...
- [wloc 设置地理位置](https://www.icloud.com/shortcuts/faf0253279934196a53ab14e2a6d6f2d)
- [wloc 清理恢复位置](https://www.icloud.com/shortcuts/f42632d406504f24a2cd163af4fe012f)

设置位置指令基于原作者版本，由维护者将解析地址改为 `https://wloc.333012.xyz/api/parse` 后重新分享；恢复位置指令沿用上游 README 的链接。尚未独立复核新版指令的真机运行结果。安装后，在苹果地图选点 → 共享 → 选择设置指令；高德地图通过「分享 → 更多」调用。恢复指令用于清除保存值，不保证立即清除系统定位缓存。

### 替换旧解析服务

本仓库当前选点网页：https://wloc.333012.xyz/ 。使用该站点时，将旧解析服务的域名替换为 `wloc.333012.xyz`；解析接口为 `https://wloc.333012.xyz/api/parse`，原有查询参数和输入变量必须保留。自行部署的用户应使用自己的域名。
